...
type Trie struct {
	...
	mu         sync.RWMutex
	// keysCache is a map from a string to a slice of strings.
...
func (t *Trie) Put(key string, meta interface{}) {
	t.mu.Lock()
	defer t.mu.Unlock()
	node := t.root
	for _, v := range t.getKeysFromCache(key) {
		if len(v) == 0 {
			return
		}
...
func (t *Trie) Match(prefix string) map[string]interface{} {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
// getKeysFromCache get keys from cache  splited by sep
func (t *Trie) getKeysFromCache(key string) []string {
	t.keyCacheMu.Lock()
